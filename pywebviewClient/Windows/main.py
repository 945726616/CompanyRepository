...
lib_path = os.path.join(dir_path, 'npmmex.dll')
print(lib_path, 'lib_path')
# 引入动态链接库
dll = ctypes.WinDLL(lib_path)
...
def getVideo (playUrl, videoType, download_folder_path):
    global mecobj_pointer, videoTypeFlag, playUrlGlobal, downloadFolderPathGlobal, download_continue_flag, query_flag
...
def useMecWrite(pcm_audio_data, voice_chl_id):
    ...
    # print(len(pcm_params.data), 'check_pcm_data_len')
...
def destroyVideo():
    ...
    if 'mecobj_pointer' in globals() and mecobj_pointer is not None:
        ...
        print(chl_id, 'arg2')
        destroy_result = dll.mec_chl_destroy(ctypes.byref(mecobj_pointer.contents), chl_id)
...
api = Api()
# 加载网页并在其中嵌入 base64 编码的图像数据   http://45.120.103.36:7080/dcm/version/repo/website/pkg-website-v10.5.3.2305261848/index.html?m=vimtag
window = webview.create_window('Vimtag', 'http://192.168.3.181:8080/vimtag/', js_api=api, fullscreen=True)
