  ...
  <div id='history'>
    <div id='vimtag_device_list_box'>
      <div id='history_menu_box'>
        ...
        <div id='history_menu_date' @click='menu_date_sign = !menu_date_sign'
          :style='menu_date_sign ? "color:#00a6ba" : "color:#323232"'>
          <!-- 日期 -->
          <div id='history_menu_date_img'
            :class='menu_date_sign ? "history_menu_choose_date_img history_menu_date_img" : "history_menu_date_img"'></div>
          {{ mcs_date }}
        </div>
        <div id='history_menu_filter' @click='menu_filter_sign = !menu_filter_sign'
          :style='menu_filter_sign ? "color:#00a6ba" : "color:#323232"'>
          <!-- 筛选 -->
          <div id='history_menu_filter_img'
            :class='menu_filter_sign ? "history_menu_filter_img history_menu_choose_filter_img" : "history_menu_filter_img"'>
          </div> {{ mrs_filter }}
        </div>
        <div id='history_menu_edit' @click='menu_edit_sign = !menu_edit_sign'
          :style='menu_edit_sign ? "color:#00a6ba" : "color:#323232"'>
          <!-- 编辑 -->
          <div id='history_menu_edit_img'
            :class='menu_edit_sign ? "history_menu_choose_edit_img history_menu_edit_img" : "history_menu_edit_img"'></div>
          {{ mcs_edit }}
        </div>
        <div id='calendar_input' v-show='menu_date_sign'></div>
      </div>
      <transition name='slide'>
        <div id='filter_menu_box' v-show='menu_filter_sign'>
          ...
          <div class='filter_list' v-if='filter_type.format_options != 1'>
            ...
            <div id='time_length_30min' class='filter_btn filter_time_btn filter_btn_active' time_length='30min'>
              <label class='radiobox' @change="time_length_30min_btn">
                <input name='currentRadio2' type='radio' :checked='filter_type.time_length == "30min"'>
                {{ mcs_half_hour }}
              </label>
            </div>
      ...
      <div id='history_list_main' v-if='history_data && history_data.length > 0'>
        <div v-for='(item, index) in history_data' :key='index' class='history_list_img' style='position:relative;'
          :num='index' img='false'>
          ...
          <div class='camera_sign_picture' v-if='!item.is_photo' type='video'
            :pic_token='item.cut_video_data[0].pic_token' :download_token='item.cut_video_data[0].token'
            :token='item.cut_video_data[0].token + item.end_new_token' :start_time='item.time_start'
            :end_time='item.time_end' @click='camera_sign_picture_btn'>
            <img class='video_list_picture' :num='index'>
          </div>
          <div class='camera_sign_picture' v-else type='photo' :pic_token='item.cut_video_data[0].pic_token'
            :token='item.cut_video_data[0].token' :start_time='item.time_start' :end_time='item.time_end'
            @click='photo_sign_picture_btn'>
            <img class='video_list_picture' :num='index'>
          </div>
...
export default {
  ...
  methods: {
    create_history_list (data) { //创建历史列表
      ...
      let loopTime = 0
      console.log(_this.history_data, '_this.history_data')
