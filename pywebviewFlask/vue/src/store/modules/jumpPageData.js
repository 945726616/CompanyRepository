...
const jumpPageData = {
  state: {
    pageDom: '',
    pageObj: {},
    projectName: localStorage.getItem('projectName') ? localStorage.getItem('projectName') : getProjectName(), // 项目名称获取,考虑后续优化获取方法,并删除相关修改项
    localFlag: 0, // 本地模式标识
    loginWaitFlag: 0,
...
async function getProjectName () { // 获取项目名称从域名中截取并存储在session中
  let returnItem
  let url = window.location.href
  window.addEventListener('pywebviewready', async () => {
    pywebview.api.setProjectName().then((res) => {
      if (res) {
        if (res.indexOf('vimtag')){
          returnItem = 'vimtag'
        } else if (res.indexOf('mipcm') > 1) {
          returnItem = 'mipcm'
        } else if (res.indexOf('ebitcam') > -1) {
          returnItem = 'ebitcam'
        } else {
          returnItem = 'vsmahome'
        }
  ...
  localStorage.setItem('projectName', returnItem)
  return returnItem
}