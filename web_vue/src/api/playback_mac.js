...
const playback_mac = {
  ...
  ** python下载视频接口
  */
  async python_download(data) {
    console.log(data, 'download_data')
    let proto = 'rtdp'
    await axios.get('/ccm/ccm_replay', { // 客户端调用回放下载接口
      params: {
        sess: {
          nid: login.create_nid(),
          sn: data.sn
      ...
      if (res && res.data.url) {
        window.pywebview.api.begin_download(res.data.url, data.download_path)
      }
    })
  },
  /*
      ...
      switch (obj.type) {
        ...
        case "install_ui": {
          ...
          obj.panel.innerHTML = "<div id='plugin_install_box' style='" + (data.ipc_stat === 0 ? 'display:none' : '') + "'>"
            ...
            + "<a name='flash' href='javascript:;'><div id='use_ordinary_video'>" + mcs_temporarily_installed_use_ordinary_video + "</div></a>"
            + "</div>"
    ...
    function flash_play (i) {
      ...
      if (!playbackFlag) {
        ...
        if (process.env.NODE_ENV === 'production') {
          ...
          urls = "http://localhost:8080/api/ccm/ccm_pic_get.jpg?hfrom_handle=887330&dsess=1&dsess_nid=" + login.create_nid() + "&dsess_sn=" + data.sn + "&dtoken=" + pic_token + "&dencode_type=0&dpic_types_support=2&dflag=2"//"&dencode_type=3&dpic_types_support=2";
        }
      }
  ...
  */
  play_preview_img (data) {
    let url;
    if (process.env.NODE_ENV === 'production') {
      url = (data.addr ? "http://" + data.addr : window.location.protocol + "//" + window.location.host) + "/ccm/ccm_pic_get.js?dsess=1&dsess_nid=" + login.create_nid() + "&dsess_sn=" + data.sn + "&dtoken=" + data.pic_token + "&dencode_type=0&dpic_types_support=2&dflag=2"//"&dencode_type=3&dpic_types_support=2";
    } else {
