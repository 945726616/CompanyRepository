...
export default {
  ...
  methods: {
    vimtagPlay (obj) {
      ...
      // }
      // 动态设置播放器大小
      this.playViewWidth = Math.floor((document.documentElement.clientWidth - 17 - 100) - (parseInt($("#play_dev_list").css("width")) + 20))
      this.playViewHeight = Math.floor(this.playViewWidth / 16 * 9 + 44)
  ...
  async mounted () {
    ...
    // 测试页面间通讯
    // 页面加载完成时获取播放器窗口定位, 并发送至客户端
    // 不使用play_screen获取, 由于play_screen经过定位获取到的坐标值为(0, 0). 所以使用play_view进行基础定位
    let playerOrigin = {
      x: Math.floor(document.getElementById('play_view').getBoundingClientRect().left),
      y: Math.floor(document.getElementById('play_view').getBoundingClientRect().top),
      width: this.playViewWidth,
      height: this.playViewHeight - 44
    }
    console.log(playerOrigin, 'playerOrigin')
    window.electronAPI.setPlayerOrigin(JSON.stringify(playerOrigin))

    // 监听主进程传递的信息
    window.electronAPI.receiveMainMessage((event, value) => {
      console.log(value, 'mainProcessMessage')
      let playerObj = {
        x: Math.floor(document.getElementById('play_view').getBoundingClientRect().left),
        y: Math.floor(document.getElementById('play_view').getBoundingClientRect().top),
        width: Math.floor(document.getElementById('play_view').offsetWidth),//this.playViewWidth,
        height: Math.floor(document.getElementById('play_view').offsetHeight - 44)//this.playViewHeight - 44
      }
      console.log(playerObj, 'playerObj')
      // alert(1)
      // 根据主进程发送的value值判断返回那个信息
      switch (value) {
        case 0: {
          event.sender.send('window-maximize', JSON.stringify(playerObj))
          break
        }
        case 1: {
          event.sender.send('window-unmaximize', JSON.stringify(playerObj))
          break
        }
        case 2: {
          console.log(playerObj, 'resize playObj')
          event.sender.send('window-resize', JSON.stringify(playerObj))
          break
        }
        case 3: {
          console.log('send window-move message')
          event.sender.send('window-move', JSON.stringify(playerObj))
          break
        }
      }
    })

    // 滚动事件监听, 页面滚动事件发生后将滚动的相关数据发送至客户端
    window.addEventListener('scroll', () => {
      let sendObj
      sendObj = {
        offsetTop: document.getElementById('play_view').offsetTop,
        offsetLeft: document.getElementById('play_view').offsetLeft,
        scrollTop: document.documentElement.scrollTop
      }
      window.electronAPI.setPlayerPosition(JSON.stringify(sendObj))
      console.log(document.documentElement.scrollTop, 'get scroll event')
      console.log(document.getElementById('play_view').offsetTop, 'player top')
      console.log(document.getElementById('play_view').offsetLeft, 'player left')
    })
  },
  // 销毁方法
  destroyed () {
    window.electronAPI.setHidePlayer('true')
  },
  watch: {
    sharpness_value (val) {
      if (val || val == 0) {
